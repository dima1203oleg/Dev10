...
Latest gate run 2026-09-01: `npm run test:integration` passed the disposable RLS integration test (1/1). The live connector self-test passed official Prozorro reachability/search and cursor pagination (10 real tenders, 0 duplicates). Authenticated `/api/production/verify` passed database/auth/Prozorro/pagination and exposed Gemini HTTP 401; external tenant isolation, document pipeline and source-bound estimate gates remain unresolved.
...
Docling conversion gate 2026-09-01: `/health` returned `{"status":"ok"}` and a real two-page Prozorro PDF was accepted by the endpoint with the configured API key, but the synchronous conversion produced no response within 180 seconds and the container subsequently became unavailable. The worker now uses the official asynchronous endpoint with bounded polling; OCR/provenance remains `FAIL/BLOCKED` pending a stable conversion run.
